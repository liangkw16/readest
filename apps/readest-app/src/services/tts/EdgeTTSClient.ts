...
// platform policy around the wss -> same-origin https transport fallback.
...
export class EdgeTTSClient extends BufferedTTSClient {
  ...
  override async init(_protocol: EDGE_TTS_PROTOCOL = 'wss'): Promise<boolean> {
    ...
    // web clients fall back to the same-origin https proxy route. On Tauri
    // the native WebSocket (with full headers) is the only Edge transport —
    // a failure there means offline or Edge itself is down, so never treat a
    // relative /api route as a remote service.
    if (await this.#edgeProvider.init('wss')) {
      this.initialized = true;
      return true;
    }
    if (!isTauriAppPlatform() && (await this.#edgeProvider.init('https'))) {
      this.initialized = true;
      return true;
    }
