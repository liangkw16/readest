...
const LibraryPageContent = ({ searchParams }: { searchParams: ReadonlyURLSearchParams | null }) => {
  ...
  // Hydrate custom fonts from local settings. User-owned file sync carries
  // the settings snapshot; there is no account-backed replica pull.
  useCustomFonts();
  ...
  // Library-scoped auto-sync for user-owned storage keeps library.json current
  // on import / delete / book-close. No-op when no provider is enabled.
  useLibraryFileSync();
    ...
    async (appService: AppService, openWithFiles: string[], libraryBooks: Book[]) => {
      ...
      for (const file of openWithFiles) {
        ...
        try {
          ...
          // OS share-sheet and desktop "open with" both import locally. The
          // configured FileSyncProvider observes the library change.
  ...
  const { handleBookDownload } = useBookTransferActions(envConfig, updateBook);
  ...
  const handleBookDelete = (deleteAction: DeleteAction) => {
    return async (book: Book, syncBooks = true) => {
      ...
      try {
        ...
        // File sync observes the tombstone written above and removes the
        // user-owned remote copy on the next convergent pass. A stale
        // cloud-only action merely forgets the old upstream marker locally.
