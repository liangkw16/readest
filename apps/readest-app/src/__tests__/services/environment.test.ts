...
describe('environment', () => {
  ...
  describe('getBaseUrl', () => {
    ...
    test('returns an empty same-origin base when no server is configured', async () => {
  ...
  describe('getNodeBaseUrl', () => {
    ...
    test('returns runtime-configured nodeBaseUrl when set', async () => {
      window.__READEST_RUNTIME_CONFIG = {
        nodeBaseUrl: 'https://runtime-node.example.com',
      };
      env['NEXT_PUBLIC_NODE_BASE_URL'] = 'https://custom-node.example.com';
      const { getNodeBaseUrl } = await import('@/services/environment');
      expect(getNodeBaseUrl()).toBe('https://runtime-node.example.com');
    });

    test('returns an empty same-origin base when no node server is configured', async () => {
      delete env['NODE_BASE_URL'];
  ...
  describe('getAPIBaseUrl', () => {
    ...
    test('returns a configured full URL in production', async () => {
    ...
    test('returns a same-origin route for tauri when no server is configured', async () => {
      env['NODE_ENV'] = 'development';
      env['NEXT_PUBLIC_APP_PLATFORM'] = 'tauri';
      delete env['API_BASE_URL'];
      delete env['NEXT_PUBLIC_API_BASE_URL'];
  ...
  describe('getNodeAPIBaseUrl', () => {
    ...
    test('returns a configured full node URL in production', async () => {
      env['NODE_ENV'] = 'production';
      env['NEXT_PUBLIC_APP_PLATFORM'] = 'web';
      env['NEXT_PUBLIC_NODE_BASE_URL'] = 'https://custom-node.example.com';
      const { getNodeAPIBaseUrl } = await import('@/services/environment');
    ...
    test('returns a same-origin route for tauri when no node server is configured', async () => {
      env['NODE_ENV'] = 'development';
      env['NEXT_PUBLIC_APP_PLATFORM'] = 'tauri';
      delete env['NODE_BASE_URL'];
      delete env['NEXT_PUBLIC_NODE_BASE_URL'];
